//! Network Utilities
//!
//! Provides network connectivity utilities including HTTP JSON fetching.
//! Used primarily for update checking.
...
pub fn fetchJson(allocator: std.mem.Allocator, url: []const u8, headers: []const http.Header, io: std.Io) !std.json.Parsed(std.json.Value) {
    ...
    var req = try client.request(.GET, try std.Uri.parse(url), .{
        .headers = .{ .user_agent = .{ .override = std.fmt.comptimePrint("zigantic/{s}", .{builtin.zig_version_string}) } },
...
test "NetworkError exists" {
    // Basic compile test - verify the error type is accessible
    const T = NetworkError;
    try std.testing.expect(@typeInfo(T) == .error_set);
}
